...
class LitAWB(LightningModule):
    ...
    #     rec_loss, smooth_loss, style_loss = 0, 0, 0
    ...
    #         style_features = vgg16(targets)
    #         style_gram = [utils.gram(fmap) for fmap in style_features]
            
    #         # get vgg features
    #         y_hat_features = vgg16(pred)
            
    #         # style loss
    #         y_hat_gram = [utils.gram(fmap) for fmap in y_hat_features]
    #         style_loss = 0.0
    #         for j in range(4):
    #             style_loss += loss_mse(y_hat_gram[j], style_gram[j][:len(inputs.shape[2])])
    #         style_loss = STYLE_WEIGHT*style_loss
    #         print(style_loss)
        
    #     # pred_weights batch_size, channel, width, height vs label
    #     # style_predict = StyleModel(pred_weights)
    #     # style_label = StyleModel(targets)
            
    #     loss = (rec_loss / inputs.shape[0]) + (smooth_loss / inputs.shape[0]) + (style_loss / inputs.shape[0])
    ...
    def training_step(self, batch, batch_idx):
        ...
        for c in range(inputs.shape[1]):
            ...
            # Calculate gram matrices for style feature layer maps we care about
            style_features = self.style_model(targets.to(self.device))
            style_gram = [utils.gram(fmap) for fmap in style_features]
...
def main(args):
    ...
    # load wgg16 model
    # vgg16 = StyleModel(freeze=True).to(device)
    vgg16 = StyleModel(freeze=True).to("cpu")
    # print(vgg16)
    ...
    # model_path = "/home/tiennv/FPT/training_wb/checkpoints/WB_model_p_64_D_S_T.pth"
    
    # Load weight
    # litmodel = attem_load_author(model=litmodel, checkpoint_path=model_path)
    # map_location = torch.device('cuda:0')
    # print(torch.load(model_path, map_location=map_location))
    # litmodel.load_state_dict(torch.load(model_path, map_location=map_location)['state_dict'])
    # litmodel.to(device=args.device[0])
