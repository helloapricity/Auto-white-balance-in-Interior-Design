...
# from src.models.litawb_style_module import LitAWB
...
class AWBInference():
    ...
    def input_folder(self, data_dir, out_dir):
        ...
        for folder in tqdm(img_folders):
            ...
            inps = [os.path.join(folder, f"{number}_{i}.png") for i in range(1, 4)]
            print(f"inps: {inps}")
            ...
            with torch.no_grad():
                ...
                if self.post_process:
                    print("first weight" , weights.shape[1])
                    print("zero weight" , weights.shape[0])
